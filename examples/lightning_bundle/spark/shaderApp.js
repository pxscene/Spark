...
class SparkPlatform {
    ...
    loadSrcTexture({src}, cb) {
        ...
        imageResource.ready.then(function(res) {
        let sparkImage = sparkscene.create({t:"image", resource:res});
...
class MyApp extends ux$1.App {
    static _template() {
        return {
            shader: {type: lightning$1.shaders.Grayscale, amount: 0.9},
            LilLightning:{ x: 100, y: 50, src: MyApp.getPath("ll_image.png"), scaleX:1.5, scaleY:1.5},
            Header: {
                rect: true, w: 1920, h: 1280, color: 0xff005500,
            SubLilLightning: {
                x: 400, y: 50, src: MyApp.getPath("ll_image.png"),scaleX:1.5, scaleY:1.5,
                shader: null // Reset shader to default.
            },
            SubLilLightning2: {
                x: 400, y: 500, src: MyApp.getPath("ll_image.png"),
                shader: {type: lightning$1.shaders.Inversion} // Reset shader to other.
            },
            Scaler: {type: lightning$1.components.SmoothScaleComponent, smoothScale: 1, content: {
                text: {color: 0xff005500, text: "Scaled text", fontSize: 200}
            }},
            }
        }
    }
    ...
    _handleUp(){
        this.tag("SubLilLightning2").shader = null;
    }
    _handleDown(){
        this.tag("SubLilLightning2").shader = {'type': lightning$1.shaders.Inversion}
    }
}
...
launcher.launch(MyApp, {debug:false, h:sparkscene.h, keys:keymaps}, {useInspector: false});
