(function (global, factory) {
    ...
    class Image9Texture extends lng.Texture {
      ...
      _getSourceLoader() {
        const _this = this;
        return function(cb) {
          let supportsNativeRendering = false;
          let proxyServer = "";
          if (sparkQueryParams && sparkQueryParams.sparkProxyServer) {
              proxyServer = sparkQueryParams.sparkProxyServer;
          }
          let imageResource = sparkscene.create({t:"imageResource", url:_this._url, proxy:proxyServer});
          if (sparkscene.capabilities.sparkgl && sparkscene.capabilities.sparkgl.nativedrawing) {
            supportsNativeRendering = true;
          } 
          let sparkImage = sparkscene.create({t:"image9", resource:imageResource,
            insetLeft:_this._insetLeft, insetRight:_this._insetRight, insetBottom:_this._insetBottom, insetTop:_this._insetTop});
    
          const sparkGl = _this.stage.gl;
          sparkImage.ready.then( function(obj) {
              if (!supportsNativeRendering){
                ...
                sparkImage.drawNatively = true;
                cb(null, {source: -1, w: sparkImage.resource.w, h: sparkImage.resource.h, premultiplyAlpha: false, flipBlueRed: false, imageRef: sparkImage});
              }
            }, function(obj) {
              let canvas = {};
              canvas.internal = sparkImage;
              cb(null, canvas);
          });
    
          _this._sparkImage = sparkImage;
        }
      }
    }

    class ImageAnimatedTexture extends lng.Texture {
      ...
      _getSourceLoader() {
        ...
        return function(cb) {
          ...
          let sparkImage = null;
          if (sparkscene.capabilities.sparkgl && sparkscene.capabilities.sparkgl.animatedImages) {
            sparkImage = sparkscene.create({t:"imageA", url:_this._url, proxy:proxyServer, resolveWithoutParent:true, stretchX:1, stretchY:1});
          } else {
            sparkImage = sparkscene.create({t:"image", url:_this._url, proxy:proxyServer, resolveWithoutParent:true, stretchX:1, stretchY:1});
          }
    ...
    const location = new URL(__dirname);
    ...
    class SparkPlatform {
        ...
        paint(gl, obj, x, y, color, e) {
            if (obj) {
                ...
                let isAnimatedImage = typeof obj.description === "function" && obj.description() === "pxImageA";
        ...
        static createImage9Texture() {
          return Image9Texture;
        }

        static createImageAnimatedTexture() {
          return ImageAnimatedTexture;
        }
