...
var appBundle = function () {
  ...
  for (var _i in values) {
    var tpl = {
      x: x,
      y: y,
      text: JSON.parse(JSON.stringify(values[_i]))
    };
    tpl.text.maxLines = 4;
    template['Text_1_' + _i] = Defaults.applyToText(tpl, ['fontFace', 'fontSize', 'textColor']);
    x += 450;
  }
  /*
  ...
  for (var _i2 in values) {
    var _tpl = {
      x: x,
      y: y,
      text: JSON.parse(JSON.stringify(values[_i2]))
    };
    template['Text_2_' + _i2] = Defaults.applyToText(_tpl, ['fontFace', 'fontSize', 'textColor']);
    x += 450;
  }
  /*
