...
var appBundle = function () {
  ...
  var text_align = [false, // attribute will not be set
  'left', 'center', 'right'];
  ...
  for (var i in text_align) {
    ...
    template['Text_1_' + i] = Defaults.applyToText(tpl, ['fontFace', 'fontSize', 'textColor']);
    y += (def$1.text.fontSize || space) + space;
  }

  var max_lines = 3;

  for (var _i in text_align) {
    var _tpl = {
      x: x,
      y: y,
      text: {
    ...
    if (text_align[_i] !== false) {
      _tpl.text.textAlign = text_align[_i];
    }

    template['Text_2_' + _i] = Defaults.applyToText(_tpl, ['fontFace', 'fontSize', 'textColor', 'wordWrapWidth']);
