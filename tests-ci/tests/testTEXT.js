...
px.import({scene:"px:scene.1.js",
  ...
  //var bg = scene.create({ t: 'rect', parent: root , fillColor: '#ccc',  x: 0, y: 0, w: scene.w, h: scene.h, interactive: false });
  ...
  // var boundsRectColor = 0xFFFF0077;
  var boundsRectColor = "#ff07";

  // var charsRectColor = 0xFF00FF77;
  var charsRectColor = "#f0f7";


  // Container and widgets for news stories
  var overlayContainerNews = scene.create({t:"object",parent:bg,a:1});
  var title = scene.create({t:"textBox",parent:overlayContainerNews,y:10,x:25,h:90,w:400,pixelSize:30,textColor:textColor,
  ...
  var body = scene.create({t:"textBox",parent:overlayContainerNews,x:25,y:95,h:300,w:400,pixelSize:20,wordWrap:true,
                          textColor:textColor, clip:true, truncation:scene.truncation.TRUNCATE_AT_WORD,
                          font:bodyFont});
  ...
  var expectedValuesMeasure = [
    ...
    [[25,10,376,80,25,38,39,376,73,74],[25,10,410,92,25,42,43,370,83,84]], // test0
    [[25,10,372,80,25,38,39,92,73,74],[25,10,304,92,25,42,43,226,83,84]], // test1
    [[25,10,423,80,25,38,39,423,73,74],[25,10,360,92,25,42,43,284,83,84]], // test2
    [[25,10,393,80,25,38,39,393,73,74],[25,10,342,92,25,42,43,234,83,84]] // test3
  ...
  if(storyIndex < storiesLen)
  {
    ...
    Promise.all([title.ready, body.ready]).then( function()
    {
      ...
      then(function(c)
      {
              ...
              var boundsRect = scene.create({t:"rect", fillColor:"#000", parent:overlayContainerNews, lineColor: boundsRectColor, lineWidth:1, x:measurements.bounds.x1,   y:measurements.bounds.y1, w:measurements.bounds.x2 - measurements.bounds.x1,      h:measurements.bounds.y2 - measurements.bounds.y1});
              var charsRect  = scene.create({t:"rect", fillColor:"#000", parent:overlayContainerNews, lineColor: charsRectColor, lineWidth:1, x:measurements.charFirst.x, y:measurements.charFirst.y, w:measurements.charLast.x - measurements.charFirst.x, h:(measurements.charLast.y - measurements.charFirst.y)==0?1:(measurements.charLast.y - measurements.charFirst.y)});
