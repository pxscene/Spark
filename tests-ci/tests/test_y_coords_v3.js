/** test_y_coords_v3 is testing the same things as test_y_coords_v2, except
 * the textBoxInContainer is placed at x:25, y:25 within its parent, so bounds 
 * values change and are correct if the textBox fix indicated by 
 * scene.capabilities.graphics.text >=3 is present.
 */
...
var container = scene.create({parent:root, t:"object",x:800, y:75,w:800, h:400, clip:false});
var textBoxInContainer = scene.create({parent:container, t:"textBox", font:fontDejaVuSans, 
              w:800, h:300, textColor:0xFFFFFFFF, pixelSize:25, x:25, y:25,
...
var checkForFix = function() {
  if( scene.capabilities.graphics.text != undefined && scene.capabilities.graphics.text >= 3) {
    return true;
  }
  else 
    return false;
...
var tests = {
  ...
  return new Promise(function(resolve, reject) {
    ...
    textBox.ready.then(function(obj) {
      if( !checkForFix()) {
        console.log("textBox fix is not supported, so test_y_coords_v3.js would fail; skipping tests.");
        results.push(assert(true,"This version of test_y_coords is not supported"));
      } else {
    ...
    textBoxInContainer.ready.then(function(obj) {
      if( !checkForFix()) {
        ...
        results.push(assert(measurements.bounds.x1 == 25, "textboxInContainer bounds.x1 is "+measurements.bounds.x1+" but should be 25"));
        results.push(assert(measurements.bounds.x2 == 478, "textboxInContainer bounds.x2 is "+measurements.bounds.x2+" but should be 478"));
        results.push(assert(measurements.bounds.y1 == 25, "textboxInContainer bounds.y1 is "+measurements.bounds.y1+" but should be 25"));
        results.push(assert(measurements.bounds.y2 == 54, "textboxInContainer bounds.y2 is "+measurements.bounds.y2+" but should be 54"));
        results.push(assert(measurements.charFirst.y == 49 || measurements.charFirst.y == 50, "textboxInContainer charFirst.y is "+measurements.charFirst.y+" but should be 49 or 50"));
        results.push(assert(measurements.charLast.y == 49 || measurements.charLast.y == 50, "textboxInContainer charLast.y is "+measurements.charLast.y+" but should be 49 or 50"));
        results.push(assert(measurements.charFirst.x == 25, "textboxInContainer charFirst.x is "+measurements.charFirst.x+" but should be 25"));
        results.push(assert(measurements.charLast.x == 478, "textboxInContainer charLast.x is "+measurements.charLast.x+" but should be 478"));
        ...
        var metrics = fontDejaVuSans.getFontMetrics(obj.pixelSize);
        console.log("metrics baseline is "+metrics.baseline);
        // Metrics baseline will not reflect position of textBox
        ...
        var fontMeasurements = fontDejaVuSans.measureText(obj.pixelSize,obj.text);
        results.push( assert(measurements.bounds.x2-measurements.bounds.x1 === fontMeasurements.w, "Text width from textboxInContainer does not match font measurement width: measurements.bounds.x2-measurements.bounds.x1:"+measurements.bounds.x2-measurements.bounds.x1+" != fontMeasurements.w:"+fontMeasurements.w));
