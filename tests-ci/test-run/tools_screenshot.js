...
*/
function isDumpScreenshotEnabled()
{
  var dumpScreenShotEnabled = false;
  if( px.appQueryParams.dumpScreenShot !== undefined) {
    var param = px.appQueryParams.dumpScreenShot;
    dumpScreenShotEnabled = (param == "true")?true:false;
  }
  return dumpScreenShotEnabled;
}
...
function getPNGForValidation(validateUrl)
{
  ...
  function takeScreenshot(url, consolePrint) 
  {
      var data = imports.scene.screenshot('image/png;base64');
      var buf = new Buffer(data.slice(data.indexOf(',')+1), 'base64');
      url = url.substring(url.lastIndexOf("/")+1);
      if( consolePrint || isDumpScreenshotEnabled()) {
        console.log("VALIDATION IMAGE:"+url+":["+buf.toString('base64')+"]");
      }
